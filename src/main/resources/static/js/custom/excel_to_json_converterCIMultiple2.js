		...
		.ready(function(e) {
					...
					$(document).on('click', '.btnSendBulkEmailWithExcel', function(e) {
										...
										for (var i = 0; i < obj.length; i++) {
											...
											if(jsonbj && Object.keys(jsonbj).length !== 1){
												obj[i].colData = jsonbj;
											}
										}
					...
					function builddata(JSON_DATA) {
						...
						var keys = Object.keys(obj.Sheet1[0]);

						var columnDefinitions = keys.map(function(key) {
						    return { "sTitle": key, "mData": key };
						});
