...
CloudGenerator.prototype.createCloud = function(x) {
	...
	cloud.xSpeed = 0.2 + 0.1 * Math.random();
...
let currUrl1 = window.location.href;
let urlToset1 = currUrl1.split("/emailOrSms");
var viewport = document.getElementById('container');
var clouds = new CloudGenerator(viewport, urlToset1[0] + '/images/cloud.webp');
clouds.init();

$(document).ready(function() {
	setTimeout(function() {
		if ($(window).height() < 920) {
			$('.wrapper').css({ width: $(window).width(), height: $("#content-page").height() + $('footer').height() + $('.iq-top-navbar').height() })
		} else {
