...
import gov.cdac.emailservice.icg.officer.repository.EmailSentRepository;
import gov.cdac.emailservice.icg.officer.repository.IcgOfficerExamMasterRepository;
...
public class IcgOfficerMailThread extends Thread {
	...
	private IcgOfficerExamMasterRepository examMasterRepository = null;
	private EmailSentRepository emailSentRepository = null;

	public IcgOfficerMailThread() {
		super();
	}

	public IcgOfficerExamMasterRepository getExamMasterRepository() {
		return examMasterRepository;
	}

	public void setExamMasterRepository(IcgOfficerExamMasterRepository examMasterRepository) {
	 ...
	 * @param oneHundredEmailIdsAndHallTicketNumbers Map<String, String> contains
	 *                                               emailid and hallticket number
	 * @param admitCardFilePathFromPropertyFile      file path of the admit card
	 * @param mailShouldBeSend                       boolean value for sending the
	 *                                               mail
	 * @param applicantHallTicketService             ApplicantHallTicketService
	 */
	public IcgOfficerMailThread(String mailServerHost, String mailServerPort, Boolean starttls,
			String socketFactoryPort, String mailUserName, String mailPassword, String mailSubject, String mailContent,
			Map<String, String> oneHundredEmailIdsAndHallTicketNumbers, String admitCardFilePathFromPropertyFile,
			ArrayList<File> fileArrayList, boolean mailShouldBeSend,
			ApplicantHallTicketService applicantHallTicketService, String sendAdmitCardsRadio,
			String tempFileDirFromPropertyFile, String mailReason, IcgOfficerExamMasterRepository examMasterRepository,
		...
		centerWiseSendEmail.info(
				"oneHundredEmailIdsAndHallTicketNumbers.size() : " + oneHundredEmailIdsAndHallTicketNumbers.size());
	...
	public void run() {
		...
		centerWiseSendEmail.info("oneHundredEmailIdsAndHallTicketNumbers.size() of " + threadName + " is "
				+ oneHundredEmailIdsAndHallTicketNumbers.size());
		String emailId = null;
		...
		for (Map.Entry<String, String> emailIdAndHallTicketNumber : oneHundredEmailIdsAndHallTicketNumbers.entrySet()) {
			...
			centerWiseSendEmail.info(threadName + " " + count + " " + emailId + "/" + hallTicketNumber + "/"
					+ centerCodeFromHallTicketNumber);
			count++;

			admitCardFilePath = getAdmitCardFilePathFromHallTicketNumberAndCenterCode(hallTicketNumber,
					centerCodeFromHallTicketNumber);

			centerWiseSendEmail.info("admitCardFilePath : " + admitCardFilePath);

			if (admitCardFilePath == null) {
				centerWiseSendEmail.error("Admit card of Hall Ticket Number " + hallTicketNumber + " with EmailId "
						+ emailId + " is not present");
				...
				if (sendAdmitCardsRadio.equalsIgnoreCase("yes")) {
					if (!MailServiceImpl.sendMailCenterWise(mailServerHost, mailServerPort, starttls, socketFactoryPort,
							mailUserName, mailPassword, emailId, mailSubject, mailContent, fileArrayList,
							mailShouldBeSend, tempFileDirFromPropertyFile, admitCardFilePath)) {
					...
					if (!MailServiceImpl.sendMailCenterWise(mailServerHost, mailServerPort, starttls, socketFactoryPort,
							mailUserName, mailPassword, emailId, mailSubject, mailContent, fileArrayList,
							mailShouldBeSend, tempFileDirFromPropertyFile, null)) {
						centerWiseSendEmail.error("Mail Not Sent to : " + emailId + " due to Exception");
						centerWiseMailNotSentEmailLogger.info(emailId);
						mailNotSentSet.add(emailId);
						mailNotSentCounter++;
	...
	/*
	 * public boolean updateEmailSentFlag(Set<String> mailSentSet, boolean
	 * flagToSet) { ApplicantHallTicketServiceImpl applicantHallTicketServiceImpl =
	 * new ApplicantHallTicketServiceImpl(); return
	 * applicantHallTicketServiceImpl.updateEmailSentFlag(mailSentSet, flagToSet); }
	 */
