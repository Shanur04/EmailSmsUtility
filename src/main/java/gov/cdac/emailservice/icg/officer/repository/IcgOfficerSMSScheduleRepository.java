...
import gov.cdac.emailservice.icg.officer.pojo.IcgOfficerSMSScheduleDetail;


@Repository("icgOfficerSMSScheduleRepository")
public interface IcgOfficerSMSScheduleRepository extends JpaRepository<IcgOfficerSMSScheduleDetail, Long>{
	
	@Query(value="select * from sms_schedule_detail ss where ss.sms_sent_id = ?1 order by ss.sms_schedule_id", nativeQuery = true)
	List<IcgOfficerSMSScheduleDetail> findByIcgOfficerSMSSent(Long sentId);
	
	 public <S extends IcgOfficerSMSScheduleDetail>S save(S entity);
	 ...
	 @Query(value = "select ss.sms_sent_id, ssd.sms_schedule_id, bm.batch_name batchName, em.exam_name examName, "
	 		...
	 		+ " join batch_master bm on ss.batch_id = bm.batch_id join exam_master em on ss.exam_id = em.exam_id join sms_reason_master srm on ss.sms_reason_master_id = srm.sms_reason_master_id "
	 ...
	 IcgOfficerSMSScheduleDetail findSMSScheduleDetailSummary();
