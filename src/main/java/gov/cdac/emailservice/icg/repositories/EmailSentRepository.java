...
@Repository("icgEmailSentRepository")
public interface EmailSentRepository extends JpaRepository<IcgEmailSent, Long>{
	...
	<S extends IcgEmailSent> S save(S entity);
	
    @Query("SELECT e FROM IcgEmailSent e WHERE e.emailSentStatus = false")
    List<IcgEmailSent> getScheduledEmailSentList();
    
    @Query(value="UPDATE email_sent es SET es.email_sent_text_report_path = ?1 WHERE es.email_sent_id = ?2", nativeQuery = true)
    @Modifying
    Integer updateEmailTextReportPath(String path, Long emailSentId);
}
