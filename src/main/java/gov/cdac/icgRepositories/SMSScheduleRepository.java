...
@Repository("icgSMSScheduleRepository")
public interface SMSScheduleRepository extends JpaRepository<SMSScheduleDetail, Long>{
	
	@Query(value="select * from sms_schedule_detail ss where ss.sms_sent_id = ?1 order by ss.sms_schedule_id", nativeQuery = true)
	List<SMSScheduleDetail> findByIcgSMSSent(Long smsSentId);
	
	 public <S extends SMSScheduleDetail>S save(S entity);
	 ...
	 SMSScheduleDetail findSMSScheduleDetailSummary();
