...
@Repository("icgSMSSentRepository")
public interface SMSSentRepository extends JpaRepository<SMSSent, Long> {

    List<SMSSent> findAllByOrderBySMSSentIdDesc();
    ...
    SMSSent findTop1ByOrderBySMSSentIdDesc();
        
    @Query("SELECT s FROM SMSSent s WHERE s.smsSentStatus = false")
    List<SMSSent> getScheduleSMSSentList();
}
