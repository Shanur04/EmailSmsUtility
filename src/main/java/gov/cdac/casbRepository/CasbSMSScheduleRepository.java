...
@Repository("casbSMSScheduleRepository")
public interface CasbSMSScheduleRepository extends JpaRepository<CasbSMSScheduleDetail, Long>{
	
	@Query(value="select * from sms_schedule_detail ss where ss.sms_sent_id = ?1 order by ss.sms_schedule_id", nativeQuery = true)
	List<CasbSMSScheduleDetail> findByCasbSMSSent(Long sentId);
	
	 public <S extends CasbSMSScheduleDetail>S save(S entity);
	...
	CasbSMSScheduleDetail findSMSScheduleDetailSummary();
